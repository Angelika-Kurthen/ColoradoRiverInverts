...
source("~/ColoradoRiverInverts/WCC_BAET_1sp_Model.R")
...
temp <- readNWISdv("01473169", "00010", "2018-10-01", "2022-09-19")

# we only have one leap year, so remove 12-31-2020
temp <- temp[-823, ]
...
# we have 1 leap year that seems to be getting in the way. 
...
for (iter in c(1:iterations)) {
  ...
  meantemp <- mean(outs$Temperature)
  ...
  for (t in timestep) {
    ...
    if (t == 1) {print("t = 1")}
      ...
      for (s in degseq) {
        if(vec <= 266 & !is.na(degreeday$DegreeDay[s])) { vec <- degreeday$DegreeDay[s] + vec
        }
        else {emerg <- t - s
    ...
    if (10 > outs$Temperature[t-1]) ABAET[2,1] <- 0.1
...
means.size_May_VC <- means.size[132:163, ]

ggplot(data = means.size_May_VC, aes(x = date,
                              y = mean.size, group = 1)) +
