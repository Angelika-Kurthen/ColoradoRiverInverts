...
m_array <- array(data = NA, dim = c(26, 50))
# this is how often the disturbance occurs from once to every other week
for (j in 1:50){ # will want to repeat quite a few times
for (i in 1:26){# Randomly select dates without replacement
  ...
  discharge_df <- as.data.frame(cbind(temp$dts, discharge))
  colnames(discharge_df) <- c("temp.dts", "discharge")
  discharge_df$discharge[match(random_dates, discharge_df$temp.dts)] <- 0.3
  ...
  immediate_response <- max(which(m[,2] == 0.3))+1
