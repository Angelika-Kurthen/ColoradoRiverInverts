...
a_immediate_df <- as.data.frame(cbind(immediate, seq(0,26), rep("A", length(immediate))))
short <- rowMeans(short_array)
a_short_df <- as.data.frame(cbind(short, seq(0,26), rep("A", length(short))))

a_immediate_df$immediate <- as.numeric(a_immediate_df$immediate)
...
a_short_df$short <- as.numeric(a_short_df$short)
