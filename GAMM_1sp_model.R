...
GAMMmodel <- function(flow.data, temp.data, baselineK, disturbanceK, Qmin, extinct, iteration, peaklist = NULL, peakeach = NULL){
  # source functions
  ...
  # its speculated (Cross et al 2010) that survivorship is between 80 - 100% for GAMM in Grand Canyon - will say 90% survive, 10% baseline mortality
  ...
  for (iter in c(1:iterations)) {
    ...
    for (t in timestep) {
      ...
      #temps
      # fecundities estimated from Hynes 1955 and Hynes 1972
      # females carry eggs in brood pouch until they hatch as juveniles - 
      # thus no set oviposition place, however individuals may get stranded if close to edge when hydropeaking occurs
      
      F2 <- 8.245 * hydropeaking.mortality(lower = 0, upper = 1, h = hp[t-1])
      
      F3 <-  21.6454 * hydropeaking.mortality(lower = 0, upper = 1, h = hp[t-1])
      # # #   
      if (F2 < 0){
        F2 <- 0
      }
      ...
      # specify baseline transition probabilities
      # Stage 1 (below 7mm) = mean(juvenile survival) from Couland 
      # Stage 2 (7-9mm) = mean(adult surival) from Couland
      # Stage 3 (9+) = mean(adult surival) from Couland
      ...
      G1 = (0.82/stageduration1) *TempSurvival[t-1]
      G2 = (0.61/stageduration2)*TempSurvival[t-1]
      P1 = (1-(1/stageduration1)) *TempSurvival[t-1]
      P2 = (1-(1/stageduration2)) *TempSurvival[t-1]
      P3 = (1-(0.61))*TempSurvival[t-1] # may survival for last stage 
