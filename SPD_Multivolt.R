...
peaks <- oneyear[c(19, 43, 47, 65, 70, 76, 88, 95, 99), ]
arrows <- tibble(
  x1 = peaks$Date,
  x2 = peaks$Date,
  y1 = c(400, 1050, 2650, 750, 2200, 750, 875, 1300, 2900), 
  y2 = c(200, 850, 2450, 550, 2000, 550, 675, 1100, 2700)
)
arrowcols <- c("#4477AA", "#EE6677","#EE6677", "#228833","#228833","#228833","#CCBB44","#CCBB44","#CCBB44")
...
ggplot(data = oneyear, aes(x = Date, y = Abund, group = as.factor(MeanTemp), color = as.factor(MeanTemp)))+
  geom_line(size = 1 )+
  scale_color_manual(name = "Mean Temperature (C)", labels = c("11", "14", "17", "20"), values = c("#4477AA", "#EE6677", "#228833",  "#CCBB44"))+
