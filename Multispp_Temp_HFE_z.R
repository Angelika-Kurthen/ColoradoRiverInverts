...
results <- mclapply(temp_seq, function(te) {
  ...
  temp_results <- lapply(q_seq, function(q_val) {
    ...
    # Modify inputs
    qs <- list(HYOS = q_val, BAET = q_val, NZMS = q_val, CHIR = q_val, GAMM = q_val)
...
write.csv(Multispp_temp_abund, "Multispp_temp_abund_hfe_z_full.csv", row.names = FALSE)
write.csv(Multispp_temp_biomass, "Multispp_temp_biomass_hfe_z_full.csv", row.names = FALSE)
write.csv(MultisppS3_temp_biomass, "MultisppS3_temp_biomass_hfe_z.csv", row.names = FALSE)
...
# Write results to CSV files
write.csv(Multispp_temp_abund, "Multispp_temp_abund_hfe_z.csv", row.names = FALSE)
write.csv(Multispp_temp_biomass, "Multispp_temp_biomass_hfe_z.csv", row.names = FALSE)
...
results <- mclapply(temp_seq, function(te) {
  ...
  temp_results <- lapply(q_seq, function(q_val) {
    ...
    means.biomass <- multispp.data.frame(out$biomass, burnin = 260, iteration = 2, value = "biomass")
...
write.csv(Multispp_temp_abund_spike, "Multispp_temp_HFE_abund_spike_z_full.csv", row.names = FALSE)
write.csv(Multispp_temp_biomass_spike, "Multispp_temp_HFE_biomass_spike_z_full.csv", row.names = FALSE)
write.csv(MultisppS3_temp_biomass_spike, "MultisppS3_temp_HFE_biomass_spike-z.csv", row.names = FALSE)
...
write.csv(Multispp_temp_abund_spike, "Multispp_temp_HFE_abund_spike_z.csv", row.names = FALSE)
write.csv(Multispp_temp_biomass_spike, "Multispp_temp_HFE_biomass_spike_z.csv", row.names = FALSE)
