...
cshort <- ggplot(data = short_df, aes(all.dates, short_mean, group = 1))+
  geom_line()

cresil <- ggplot(data = resil_df, aes(all.dates, resil_mean, group =1))+
  geom_line()

clong <- ggplot(data = long_df, aes(all.dates, long_mean, group = 1))+
  geom_line()


length(rollmean(short_df$short_mean, 3))
