...
# specify baseline transition probabilities for each species, from Carson et al., 2011
...
for (iter in c(1:iterations)) {
  ...
  for (t in timestep) {
    ...
    #---------------------------------------------------------
    # Calculate transition probability rates based on water temps
     if (temps < 19){
      G1_BAET = 0.0018 # for spring (so Jan - June BUT note that in CA temps are higher so growth rate might be higher)
      G2_BAET = 0.01239167    # transition
      P1_BAET = 0.0272 #remain
      P2_BAET = 0.3022083
      P3_MYCA = 0.869872
    } else {
      G1_BAET = 0.002033333 #fall (so July - Dec BUT note that in CA temps are higher so growth rate might be higher)
      G2_BAET = 0.003908333   # transition
      P1_BAET = 0.02536667 #remain
      P2_BAET = 0.2882917
      P3_MYCA = 0.892922
    }
    ...
    # https://seatemperature.net/current/united-states/carlsbad-california-united-states-sea-temperature, average temp about 66F
      F2_MYCA = 14.33 
      F3_MYCA = 22.73  
    } else {
      F2_MYCA = 10.98
