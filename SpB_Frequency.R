...
b_immediate_df <- as.data.frame(cbind(immediate, seq(0,26), rep("B", length(immediate))))
short <- rowMeans(short_array)
b_short_df <- as.data.frame(cbind(short, seq(0,26), rep("B", length(short))))

b_immediate_df$immediate <- as.numeric(b_immediate_df$immediate)
...
b_short_df$short <- as.numeric(b_short_df$short)
