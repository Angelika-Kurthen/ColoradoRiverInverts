...
b_immediate_df <- as.data.frame(cbind(immediate, seq(1:26), rep("B", length(immediate))))
short <- rowMeans(short_array)
b_short_df <- as.data.frame(cbind(short, seq(1:26), rep("B", length(short))))
long <- rowMeans(long_array)
b_long_df <- as.data.frame(cbind(long, seq(1:26), rep("B", length(long))))

# b_imm <- ggplot(data = immediate_df, aes(x = V2, y = immediate/10000))+
...
# 
# 
# b_short <- ggplot(data = short_df, aes(x = V2, y = short/10000))+
#   geom_line(size = 1, col = "#228833")+
#   xlab("Frequency of pulse disturbance per year")+
#   ylab("Sp B abundance relative to K")+
#   theme_bw()+
...
# 
# 
# b_long <- ggplot(data = long_df, aes(x = V2, y = long/10000))+
#   geom_line(size = 1, col = "#228833")+
#   xlab("Frequency of pulse disturbance per year")+
#   ylab("Sp B abundance relative to K")+
#   theme_bw()+
