...
out <- NZMSmodel(flow.data = flow.magnitude$Discharge, temp.data = temps, disturbanceK = 9000, baselineK = 10000, Qmin = 0.25, extinct = 50, iteration = 9, peaklist = 0.17, peakeach = length(temps$Temperature))
...
NZMS.samp.sum <- na.omit(as.data.frame(cbind(as.Date(means.list.NZMS$date, format = "%Y-%m-%d"), means[51:406])))
...
ggplot(data = means.list.NZMS, aes(x = date, y = log(mean.abund), group = 1, color = "Model")) +
  geom_line(show.legend = T, linewidth = 1, alpha = 0.8) +
  geom_line(data = NZMS.samp.sum[-125,], aes(x =V1, y = (log(V2)), color = "Empirical"), linewidth = 1, alpha = 0.8, show.legend = T)+
  geom_point(data = NZMS.samp.sum[125,], aes(x = V1, y = (log(V2)), color = "Empirical"), show.legend = T)+
