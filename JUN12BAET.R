...
BAETmodel <- function(flow.data, temp.data, baselineK, disturbanceK, Qmin, extinct, iteration, peaklist = NULL, peakeach = NULL){
  ...
  for (iter in c(1:iterations)) {
    ...
    for (t in timestep) {
      ...
      F3 = 1104.4 * hydropeaking.mortality(0.0, 0.2, h = hp[t-1]) 
      ...
      if (t > 19) {
        ...
        F3 <- (200*size)+200 * hydropeaking.mortality(0.0, 0.2, h = hp[t-1]) 
      ...
      if (5 > temps$Temperature[t-1]) {
        P1 <- 1-(1/12) *TempSurvival[t-1]
        P2 <- P1 
        G1 <- 0.37/12 * TempSurvival[t-1]
        G2 <- G1 
