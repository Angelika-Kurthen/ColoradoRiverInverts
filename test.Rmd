...
library(ggplot2)
ggplot(fit_df, aes(x = y, fill = data)) + geom_histogram() # not getting all the 0s and missing the really high #s
```
Summary of my runs
...
fit_df <- data.frame(y = c(c(unlist(site_mat)), c(unlist(y.rep))),
                     data = rep(c("Observed", "Simulated"), each = length(site_mat)))
library(ggplot2)
ggplot(fit_df, aes(x = y, fill = data)) + geom_histogram() #still not getting all the 0s and missing the really high #s
```
```{r}
library(rstanarm)
