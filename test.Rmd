...
plot(NZMSsamp$Date, log(NZMSsamp$CountTotal), type = "l")
```
...
site_covs<- as.matrix(cbind(site_intercept, flows, temperature)) #flows,temperature, circdate)
...
#
# Nmix_fit_UI <- jagsUI::jags(data = jags_data, inits = jags_inits, parameters.to.save = parameters, model.file = "N-mixturePoisNZMS.jags",  n.chains = nc, n.iter = ni, n.burnin = nb, n.thin = nt, parallel = TRUE)
# #
# print(Nmix_fit_UI)
#
# zm = coda.samples(Nmix_fit, variable.names = c("alpha", "beta", "lambda", "N" ), n.iter = ni, n.thin = nt)
#
#
...
When running the Poison N-mix Model with $\alpha$ ~ Norm(0, 0.1) and $\beta$ ~ Norm(0, 0.1) and initial values for N = maximum value at each site, $\alpha$ from a Uniform(-1, 1), and $\beta$ from a Uniform(-1, 1), DIC = 19346231, and mean $\chi^2$ for the observed and expected is 200000. 

When running the Poison N-mix Model with $\alpha$ ~ Norm(0, 1) and $\beta$ ~ Norm(0, 1) and initial values for N = maximum value at each site, $\alpha$ from a Uniform(-1, 1), and $\beta$ from a Uniform(-1, 1), DIC = 23081874, and mean $\chi^2$ for the observed and expected is 206378.8. Previous one was better
...
sink()
```
Now we include a zero-inflated Poisson to try to capture more of those 0s we aren't capturing with the model by using z as a conditional - occupied or not occupied where z ~ Bernoulli($\omega$) and we pull $\omega$ ~ beta(1,1) from a beta distribution. We include this z in the Poisson equation such that N[j] ~ Poisson(lambda * z[timestep]). 
...
Nmix_fit_UI <- jagsUI::jags(data = jags_data, inits = jags_inits, parameters.to.save = parameters, model.file = "N-mixtureZIPNZMS.jags",  n.chains = nc, n.iter = ni, n.burnin = nb, n.thin = nt, parallel = TRUE)
# 
print(Nmix_fit_UI)
...
```
<!-- The fit isn't much better. DIC is 20658554 -->
...
<!-- We can also try to include an overdispersion parameter to try to capture the fact that we still aren't reaching those max numbers with our ZIP. We will include a random sample effect to try to include that variation. This will incorporate in the model in the likelihood function where logit(p[r,j]) = beta0 + beta*predictors + sample_offset + sample_$\epsilon$ where $\epsilon$ ~ Normal(0, $\tau$) and $\tau$ = 1/sd^2 where sd ~ Uniform(0, 3). This is from https://rushinglab.github.io/WILD6900/articles/n-mixture2.html.  -->
...
<!-- Nmix_fit_UI <- jagsUI::jags(data = jags_data, inits = jags_inits, parameters.to.save = parameters, model.file = "N-mixtureZIPoverdispNZMS.jags",  n.chains = nc, n.iter = ni, n.burnin = nb, n.thin = nt, parallel = TRUE) -->
...
<!-- ggplot(fit_df, aes(x = y, fill = data)) + geom_histogram() + facet_grid(.~data) #still not getting all the 0s and missing the really high #s -->
<!-- ``` -->
<!-- Ok, says the fit is good (DIC = 6510.192), but clearly not the best if we look at observed vs simulated.  -->

<!-- I feel like the next step is to try with overdispersion parameter again () -->
...
<!-- Nmix_fit_UI <- jagsUI::jags(data = jags_data, inits = jags_inits, parameters.to.save = parameters, model.file = "N-mixturePoisoverdispNZMS.jags",  n.chains = nc, n.iter = ni, n.burnin = nb, n.thin = nt, parallel = TRUE) -->
...
<!-- ggplot(fit_df, aes(x = y, fill = data)) + geom_histogram() + facet_grid(.~data) #still not getting all the 0s and missing the really high #s -->
```
<!-- Ok, fit is about the same as the ZIP with overdispersion. Now we can try with Neg Binomial instead of Poisson in the N mix model.  -->
...
<!-- ``` -->
<!-- Bad news, throws and error when less than 2 observations per site -->
...
<!-- dim(lam) -->
<!-- dim(th) -->
<!-- is.nan(rnbinom(30000, prob = th / (th + lam[,1]), size = th )) -->
...
<!-- Nmix_fit_UI <- jagsUI::jags(data = jags_data, inits = jags_inits, parameters.to.save = parameters, model.file = "N-mixtureNBNZMS.jags",  n.chains = nc, n.iter = ni, n.burnin = nb, n.thin = nt, parallel = TRUE) -->

<!--  print(Nmix_fit_UI) -->
<!-- ``` -->
<!-- Worse fit than with ZIP so we try another version -->
...
<!--   N[r] ~ dnegbin(s[r], th) -->
<!--   s[r] <- th / (th + lambda[r]) -->
<!--   log(lambda[r]) <- alpha_0 + alpha_site[r] + sum(alpha * XN[r, ]) #XN is matrix of site covariates -->
<!--     alpha_site[r] ~ dnorm(0, sigma_site) -->
<!-- } -->

<!-- sigma_site ~ dgamma(1,1) -->
<!-- alpha_0 ~ dgamma(0.01, 0.01) -->
<!-- th ~ dgamma(0.01, 0.01) -->
<!-- phi <- 1/th -->
<!-- theta <- th -->
...
<!-- for (r in 1:R){ -->
<!--   for (j in 1:J){ -->
<!--     logit(p[r,j]) <- beta_0 + sum(off[r, j] + (beta * Xp[r,j,])) -->
<!--     y[r,j] ~ dbinom(p[r,j], N[r]) -->
<!--   } -->
<!-- } -->
...
<!-- sink() -->
<!-- ``` -->
<!-- Did not work - same log density error -->
<!-- ```{r} -->
<!-- library(rjags) -->
<!-- library(MCMCvis) -->
<!-- jags_data <- list(y = (site_mat), -->
...
<!-- jags_inits <- function(){ -->
<!--   list( -->
<!--   alpha_0 = runif(1, 0.5, 0.7), -->
<!--   beta_0 = runif(1, 0.5, 0.7), -->
<!--   sigma_site = runif(1, 0.5, 1), -->
<!--   N = apply(jags_data$y, 1, max, na.rm=TRUE), -->
<!--   alpha=runif(nAlpha,-1,1), -->
<!-- 	beta=runif(nBeta,-1,1))} -->

<!-- parameters <- c("alpha", "beta", "lambda", "p", "N", "alpha_0", "fit", "fit_new") -->
...
<!-- for (r in 1:R){ -->
<!--   for (j in 1:J){ -->
<!--     logit(p[r,j]) <- sum(off[r, j], (beta * Xp[r,j,])) -->
