...
ID <- as.numeric(as.factor(temp$Time))-1
# want it to be every 14 days, hence the 14
ID <- ID %/% 14

# aggregate over ID and TYPE for all numeric data.
outs <- aggregate(temp$Temperature[sapply(temp$Temperature,is.numeric)],
                  by=list(ID),
                  FUN=mean)
...
outs$dts <- as.Date(temp$Day[(outs$dts*14)+1], origin = "1970-01-01")
...
# perhaps we can repeat for 5 years?

degreeday <- aggregate(temp$Temperature[sapply(temp$Temperature,is.numeric)],
                       by=list(ID),
                       FUN=sum)
names(degreeday)[1:2] <-c("dts","DegreeDay")
# add the correct dates as the beginning of every period
# 
degreeday$dts <- as.Date(temp$Day[(degreeday$dts*14)+1], origin = "1970-01-01")
# order by date in chronological order
...
timestep <- seq(2, (length(outs$Temperature)), by = 1) # OR
...
for (iter in c(1:iterations)) {
  ...
  for (t in timestep) {
    ...
    if (10 > outs$Temperature[t-1]) ABAET[2,1] <- 0.01
    ...
    if (outs$Temperature[t-1] > 13) ABAET[1,1] <- 0.01
...
Vannote_Sweeney_Fig7$Week[6:15] <- Vannote_Sweeney_Fig7$Week[6:15] + 52
plot(Vannote_Sweeney_Fig7$Week, Vannote_Sweeney_Fig7$Mean)
Vannote_Sweeney_Fig7$Week <- as.POSIXct(ymd("2023-01-01") + weeks(Vannote_Sweeney_Fig7$Week))


adults <- as.data.frame(cbind(as.POSIXct(outs$dts[44:70]), sizelist[44:70], output.N.list[44:70,3,1], emergetime[44:70]))
adults$V1 <- as.POSIXct(adults$V1, origin = "1970-01-01", format = "%Y-%m-%d")

p <- ggplot(Vannote_Sweeney_Fig7, aes(x = Week, y = Mean, pch = X))+
  geom_point()+
...
p + geom_line(mapping = aes(x = adults$V1, y = adults$V2))
...
adults <- repdf[which(repdf$stage == "S3"), ]
adults <- as.data.frame(cbind(adults, rep(outs$dts, times = iterations)))

names(adults) <- c('timestep', 'stage', 'rep', 'abund', "date")
