############################
# Make partial eta files for multispp temp --> run on HPC
################################
...
mclapply(z_seq, function(z_val) {
  ...
  fname <- file.path(paste0("Multispp_Temp_Hyd", "_Z", z_val, ".rds"))
...
mclapply(z_seq, function(z_val) {
  ...
  fname <- file.path(paste0("Multispp_Temp_Hyd_Spike", "_Z", z_val, ".rds"))
  dir.create(dirname(fname), recursive = TRUE, showWarnings = FALSE)
  saveRDS(combined_df, file = fname)
