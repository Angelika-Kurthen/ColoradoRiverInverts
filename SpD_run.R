...
for (d in 1:length(all.dates)){ # 30 reps takes 60 mins
  ...
  source("D_1sp_Model.R")
  # run model
  out <- Dmodel(discharge, dates, baselineK = 10000, disturbanceK = 40000, Qmin = 0.25, extinct = 50, iteration = 9, peaklist = 0, peakeach = length(temp$Temperature))
...
jday.plot <- ggplot(data = jday.df, aes(x = all.dates, y = jday_max/10000, group = 1))+
  ...
  theme_bw()+
  ylab('Max Values of D Spp Abundance Relative to Baseline K') +
  xlab("Julian Date")
