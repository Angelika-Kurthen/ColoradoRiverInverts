...
for (hyd in 1:length(hydropeak)){
  ...
  # model sizes
  sizes <- BAETmodel(flow.data = flows$Discharge, temp.data = temps, baselineK = 5000, disturbanceK = 40000 , Qmin = 0.1, extinct = 50, iteration = 2, peaklist = hydropeak[hyd], peakeach = length(temps$Temperature), stage_output = "size")
  # model abundances
  out <- BAETmodel(flow.data = flows$Discharge, temp.data = temps, baselineK = 5000, disturbanceK = 40000 , Qmin = 0.1, extinct = 50, iteration = 2, peaklist = hydropeak[hyd], peakeach = length(temps$Temperature))
  ...
  means[hyd] <- mean(means.list.BAET$mean.abund)  
  # calculate the standard deviation of mean abundances at each hydropeaking intensity
  sd[hyd] <- sd(means.list.BAET$mean.abund, na.rm = T)
}

# compile abundance data
BAET_hyd_means <- as.data.frame(cbind(hydropeak, means, sd, rep("BAET", length(means))))
