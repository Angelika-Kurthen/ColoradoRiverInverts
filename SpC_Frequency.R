...
c_immediate_df <- as.data.frame(cbind(immediate, seq(0,26), rep("C", length(immediate))))
short <- rowMeans(short_array)
c_short_df <- as.data.frame(cbind(short, seq(0,26), rep("C", length(short))))

c_immediate_df$immediate <- as.numeric(c_immediate_df$immediate)
c_imm <- ggplot(data = c_immediate_df, aes(x = V2, y = immediate/10000, group = 1))+
...
c_short_df$short <- as.numeric(c_short_df$short)
c_short <- ggplot(data = c_short_df, aes(x = V2, y = short/10000, group = 1))+
