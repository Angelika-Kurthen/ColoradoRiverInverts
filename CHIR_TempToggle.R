...
for (te in 1:length(temp_seq)){
  ...
  sizes <- CHIRmodel(flow.data = flows$Discharge, temp.data = temps, baselineK = 10000, disturbanceK = 40000 , Qmin = 0.2, extinct = 50, iteration = 2, peaklist = 0.17, peakeach = length(temps$Temperature), stage_output = "size")
  # model abundances
  out <- CHIRmodel(flow.data = flows$Discharge, temp.data = temps, baselineK = 10000, disturbanceK = 40000 , Qmin = 0.2, extinct = 50, iteration = 2, peaklist = 0.17, peakeach = length(temps$Temperature))
  ...
  means[te] <- mean(means.list.CHIR$mean.abund)
  # calculate the standard deviation of mean abundances at each hydropeaking intensity
  sd[te] <- sd(means.list.CHIR$mean.abund, na.rm = T)
}

# compile abundance data
CHIR_te_means <- as.data.frame(cbind(temp_seq, means, sd, rep("CHIR", length(means))))

# compile timeseries biomass data
CHIR_te_size <- as.data.frame(cbind(temp_seq, sizemeans, sizesd, rep("CHIR", length(sizemeans))))

# compile annual production biomass data
CHIR_te_yrprod <- as.data.frame(cbind(temp_seq, S3Yrprod, S3Yrprod_sd, rep("CHIR", length(sizemeans))))
# 
