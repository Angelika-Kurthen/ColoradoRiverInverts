...
G1s <- seq(0.60, 0.76, by = 0.01)
G2s <- seq(0.27, 0.72, by = 0.01)
cortests <- vector()
combos <- vector()
  ...
  for (iter in c(1:iterations)) {
    ...
    for (t in timestep) {
      ...
      if (5 > temps$Temperature[t-1]) {
        ...
        P2 <- P1
        G1 <- g1s/20 * TempSurvival[t-1]
        G2 <- g2s/20 * TempSurvival[t-1]
      }
      if (temps$Temperature[t-1] > 30){
        P1 <- 0
        P2 <- 0
        G1 <- g1s* TempSurvival[t-1]
        G2 <- g2s* TempSurvival[t-1]
      }
      
      if (5 <= temps$Temperature[t-1] & temps$Temperature[t-1] <= 30 & (is.na(emergetime[t-1]) == F)){
        G1 <- (g1s/((emergetime[t-1])/2)) * TempSurvival[t-1]
        G2 <- (g2s/((emergetime[t-1])/2)) * TempSurvival[t-1]
        P1 <- (1-(1/((emergetime[t-1])/2))) * TempSurvival[t-1]
        P2 <- P1
      }
      if (5 <= temps$Temperature[t-1] & temps$Temperature[t-1] <= 30 & (is.na(emergetime[t-1]) == T)) {
        G1 <- (g1s/((-0.136 * temps$Temperature[t-1]) + 5.088)) * TempSurvival[t-1]
        P1 <- (1-(1/((-0.136 * temps$Temperature[t-1]) + 5.088))) * TempSurvival[t-1]
        G2 <- (g2s/((-0.136 * temps$Temperature[t-1]) + 5.088)) * TempSurvival[t-1]
        P2 <- P1
      }
...
out <- CHIRmodel(flow.data = flow.magnitude$Discharge, temp.data = temps, disturbanceK = 40000, baselineK = 10000, Qmin = 0.25, extinct = 50, iteration = 1000, peaklist = 0.17, peakeach = length(temps$Temperature))
...
colnames(means.list.CHIR) <- c('timesteps', 'mean.abund', 'sd.abund', 'se.abund', "Date")
