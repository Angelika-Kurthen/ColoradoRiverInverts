...
for (i in 1:length(press_magnitudes)){
  ...
  out <- Amodel(discharge, temp, baselineK = 10000, disturbanceK = 40000, Qmin = 0.25, extinct = 50, iteration = 1, peaklist = press_magnitudes[i], peakeach = length(temp$Temperature))
  m <- rowSums(out)
