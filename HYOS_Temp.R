...
results <- mclapply(temp_seq, function(te) {
  ...
  return(list(HYOS_temp_abund = average_means, HYOS_temp_biomass = average_size, HYOS_temp_percapita = percapita_biomass))
...
results <- mclapply(temp_seq, function(te) {
  ...
  # Return results as a list
  return(list(HYOS_temp_abund_spike = average_means, HYOS_temp_biomass_spike = average_size, HYOS_temp_percapita_spike = percapita_biomass))
}, mc.cores = detectCores() - 1)
